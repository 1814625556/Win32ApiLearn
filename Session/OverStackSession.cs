...
namespace Session
{
    public static class OverStackSession
    {
        #region DLL Imports
        ...
        public static bool ExecuteAppAsLoggedOnUser(string AppName, string CmdLineArgs)
        {
            ...
            if (OpenProcessToken(Process.GetCurrentProcess().Handle, TOKEN_ADJUST_PRIVILEGES, ref LoggedInUserToken)==false)
            {
            ...
            if (null != explorerProcessList && explorerProcessList.Count > 0)
            {
                foreach (Process explorerProcess in explorerProcessList)
                {
                    ...
                    try
                    {
                        var tokenRights = MAXIMUM_ALLOWED; //TOKEN_ADJUST_PRIVILEGES | TOKEN_QUERY | TOKEN_ASSIGN_PRIMARY | TOKEN_DUPLICATE | TOKEN_ADJUST_DEFAULT | TOKEN_ADJUST_SESSIONID;
                        ...
                        if (!CreateProcessAsUser(DuplicateToken, AppName, (CmdLineArgs == null) ? string.Empty : CmdLineArgs, 
                            ref processAttributes, ref threadAttributes, true, dwCreationFlags, UserEnvironment, 
                            AppName.Substring(0, AppName.LastIndexOf('\\')), ref si, out pi))
                        {
        ...
        public static bool ExecuteAppAsLoggedOnUserBak(string AppName, string CmdLineArgs)
        {
